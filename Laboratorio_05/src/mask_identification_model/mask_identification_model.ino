...
/* Includes ---------------------------------------------------------------- */
#include <Laboratorio_05_inferencing.h>
...
// Function that controls RGB LED depending on the model result
void rgb_led_control (float model_result[]); 
...
void loop()
{
    ...
    // Print the prediction results (classification)
      ...
      for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
          ...
          model_result[i] = result.classification[i].value; 
      }
      rgb_led_control(model_result); 
#endif
 ...
 *
 * @param[in]  model_results model result vector 
 *
 */
void rgb_led_control(float model_resul[])
{
  if (model_resul[0] > model_resul[1] && model_resul[0] > model_resul[2]){
    digitalWrite(LEDR, HIGH);
    digitalWrite(LEDG, HIGH);
    digitalWrite(LEDB, LOW);
  }
  
  else if (model_resul[1] > model_resul[0] && model_resul[1] > model_resul[2]){
    digitalWrite(LEDR, HIGH);
    digitalWrite(LEDG, LOW);
    digitalWrite(LEDB, HIGH);
  }

  else if (model_resul[2] > model_resul[0] && model_resul[2] > model_resul[1]){
    digitalWrite(LEDR, LOW);
    digitalWrite(LEDG, HIGH);
    digitalWrite(LEDB, HIGH);
  }
